    ...
    public async Task<List<ArtistDTO>> getArtist(QueryArtist queryObject)
    {
        ...
        switch (sortBy)
        {
            case ArtistSearchByEnum.AristName:
                list = list.Where(a => a.ArtistName.Contains(search ?? "", StringComparison.OrdinalIgnoreCase))
                    .ToList();
                list = isDesc
                    ? list.OrderByDescending(a => a.ArtistName).ToList()
                    : list.OrderBy(a => a.ArtistName).ToList();
                break;
            ...
            case ArtistSearchByEnum.NumberOfFollower:
                list = list.Where(a => a.ArtistName.Contains(search ?? "", StringComparison.OrdinalIgnoreCase))
                    .ToList();
                list = isDesc
                    ? list.OrderByDescending(a => a.NumberOfFollower).ToList()
                    : list.OrderBy(a => a.NumberOfFollower).ToList();
                break;
            default:
                list = list.Where(a => a.ArtistName.Contains(search ?? "", StringComparison.OrdinalIgnoreCase))
                    .ToList();
